...
namespace Microsoft.Azure.WebJobs.Logging.ApplicationInsights
{
    ...
    internal class WebJobsRoleEnvironmentTelemetryInitializer : ITelemetryInitializer
    {
        ...
        private const string EventName = nameof(WebJobsRoleEnvironmentTelemetryInitializer);
        private static readonly DiagnosticListener _source = new DiagnosticListener(string.Concat(ApplicationInsightsDiagnosticConstants.ApplicationInsightsDiagnosticSourcePrefix, nameof(WebJobsRoleEnvironmentTelemetryInitializer)));
        ...
        public void Initialize(ITelemetry telemetry)
        {
            try
            {
                ...
                {
                    _source.Write(EventName, "WebJobsRoleEnvironmentTelemetryInitializer, found a request telemetry -" + telemetry.GetType().Name + " - " + JsonConvert.SerializeObject(request, settings) + "------------" + act);
                }
                else
                {
                    _source.Write(EventName, "WebJobsRoleEnvironmentTelemetryInitializer Beginning- " + telemetry.GetType().Name + " - " + JsonConvert.SerializeObject(telemetry, settings) + "------------" + act);
                }
                ...
                _source.Write(EventName, "WebJobsRoleEnvironmentTelemetryInitializer End- " + telemetry.GetType().Name + " - " + JsonConvert.SerializeObject(telemetry, settings) + "------------" + act);
            }
            catch (Exception ex)
            {
                _source.Write(EventName, "WebJobsRoleEnvironmentTelemetryInitializer Exception - " + ex.Message);
            }
        }
