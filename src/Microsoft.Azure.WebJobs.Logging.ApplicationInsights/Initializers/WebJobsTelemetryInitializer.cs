...
namespace Microsoft.Azure.WebJobs.Logging.ApplicationInsights
{
    ...
    {
        private const string EventName = nameof(WebJobsTelemetryInitializer);
        private static readonly DiagnosticListener _source = new DiagnosticListener(string.Concat(ApplicationInsightsDiagnosticConstants.ApplicationInsightsDiagnosticSourcePrefix, nameof(WebJobsTelemetryInitializer)));
        ...
        public void Initialize(ITelemetry telemetry)
        {
            try
            {
                ...
                if (telemetry is RequestTelemetry r)
                {
                    _source.Write(EventName, "WebJobsTelemetryInitializer, found a request telemetry -" + telemetry.GetType().Name + " - " + JsonConvert.SerializeObject(r, settings) + "------------" + act);
                }
                else
                {
                    _source.Write(EventName, "WebJobsTelemetryInitializer Beginning-" + telemetry.GetType().Name + " - " + JsonConvert.SerializeObject(telemetry, settings) + "------------" + act);
                }
                ...
                _source.Write(EventName, "WebJobsTelemetryInitializer End-" + telemetry.GetType().Name + " - " + JsonConvert.SerializeObject(telemetry, settings) + "------------" + act);
            }
            catch (Exception ex)
            {
                // If we fail to initialize telemetry, we don't want to throw an exception
                // as it will prevent the telemetry from being sent.
                _source.Write(EventName, new { Message = "WebJobsTelemetryInitializer failed", Exception = ex });
            }
        }
